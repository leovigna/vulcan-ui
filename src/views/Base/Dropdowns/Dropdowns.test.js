...
it('toggle click without crashing', () => {
    ...
    for (let i = 0; i < 5; i++) {
        const Dropdown = wrapper.find('button.dropdown-toggle').at(i);
        Dropdown.simulate('click');
        expect(wrapper.state().dropdownOpen[i]).toEqual(true);
    }
    for (let i = 0; i < 2; i++) {
        const Dropdown = wrapper.find('[data-toggle="dropdown"]').at(0);
        Dropdown.simulate('click');
        expect(wrapper.state().dropdownOpen[5]).toEqual(true);
        const DropdownItem = wrapper.find('div.dropdown-menu > .dropdown-item').at(i);
